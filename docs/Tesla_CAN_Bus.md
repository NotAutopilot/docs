...
The Tesla CAN Bus system comprises of 6 standard CAN busses + one Ether CAN bus.
...
The Ether CAN uses UDP encapsulation to broadcast the CAN messages (8 bytes long) on port 20101 on the network between the Gateway, the CID and the IC.
...
- DAS - Auto Pilot 1 hardware (if equipped)
